...
/*   Updated: 2016/12/21 10:22:52 by saubenne         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			g_next_line(int const fd, char **line, char *buf)
{
	int					ret;
	char				*eol;
	unsigned long int	i;
	...
	*line = NULL;
	...
	|| (ret = read(fd, buf, BUFF_SIZE)) > 0)
	{
		...
		{
			*line = strnjoin(*line, buf, (ret) ? ret : i);
			ft_bzero(buf, (ret) ? BUFF_SIZE : i);
		}
		else
		{
			*eol = '\0';
			*line = strnjoin(*line, buf, eol - buf);
